...
namespace Rebex.Samples
{
    ...
    /// using .NET Framework 3.5 API and Newtonsoft.Json.
    /// </summary>
    /// <remarks>
    ...
    public class OAuthAzureCredentials
    {
        ...
        public void RequestAccessToken(string authorizationCode)
        {
            ...
            string responseJson = HttpPost(_tokenEndPoint, requestBody);

            // deserialize JSON response
            var response = DeserializeJson(responseJson);

            // OAuth 2.0 access token and expiration time
            ...
            // OAuth 2.0 refresh token - only provided with 'offline_access' scope
            ...
            if (_scopes.Contains("openid"))
            {
                ...
                // parse user name from id_token - only provided with 'profile' scope
                ...
                // parse user name from id_token - only provided with 'email' scope
        ...
        /// <returns></returns>
        private static string HttpPost(string uri, string requestBody)
        {
            ...
            catch (WebException error)
            {
                ...
                string responseJson = ReadResponseToEnd(response);
